...
const CreateBlogPage = () => {
    ...
    try {
      const blocksToSend = await Promise.all(blocks.map(async (b) => {
          ...
          try {
            ...
            const upl = await fetch(`${API_URL}/api/uploads`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: fd });
            if (!upl.ok) throw new Error('Image upload failed');
            const data = await upl.json();
            return { ...b, src: `${API_URL}${data.url}`, name: data.filename };
          } catch (err) {
  ...
  const handleImageFileChange = async (idx, e) => {
    ...
    try {
      const fd = new FormData();
      fd.append('file', file);
      const res = await fetch(`${API_URL}/api/uploads`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: fd });
  ...
  const handleDeleteImage = async (idx) => {
    ...
    if (b.name) {
      try {
        ...
        const res = await fetch(`${API_URL}/api/uploads/${filename}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
