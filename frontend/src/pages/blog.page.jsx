...
const BlogPage = () => {
  ...
  const handleBookmark = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/blogs/${blogId}/bookmark`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.status === 404) throw new Error('Post not found (server may be down)');
  ...
  const handleLikeComment = async (id) => {
    if (!token) return showToast('Please sign in to like', 'error');
    try {
      const res = await fetch(`http://localhost:5000/api/comments/${id}/like`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error('Like failed');
      await fetchComments();
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteComment = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/comments/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleReportComment = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/comments/${id}/report`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ reason }) });
...
const CommentItem = ({ comment, currentUser, onReply, onLike, onDelete, onReport }) => {
  ...
  const submitReply = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/blogs/${comment.blogId}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const adminLikePhoto = comment.adminLikePhoto
    ? (comment.adminLikePhoto.startsWith('/uploads/') ? `http://localhost:5000${comment.adminLikePhoto}` : comment.adminLikePhoto)
    : '';
